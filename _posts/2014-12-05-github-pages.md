...
- `permalink` 文章的路径，默认是`/year/month/day/title.html`。也可以自定义路径，可以用的变量有`:year`，`:month`，`:day`，`:title`以及`:categories`。`:categories`来自front matter；其他的变量都来自文章的文件名。你可以用`/:year/:month/:title`或者`/:categories/:title.html`的形式来设置permalink。需要注意的是，如果你文章的front matter中有`permalink`选项，它会覆盖掉全局的默认值
- `published` 可以单独设置某一篇文章是否需要发布
- `category` 设置文章的分类
- `tags` 设置文章的tag
- `exclude` 像上面所言，Jekyll会忽略以下划线开头的文件。但是，如果你有需要忽略的目录，但是又不是以下划线开头，你可以在`exclude`中设定

详尽的模板变量，可以参考官方的文档：[https://github.com/mojombo/jekyll/wiki/template-data](https://github.com/mojombo/jekyll/wiki/template-data "Jekyll Template Data")
