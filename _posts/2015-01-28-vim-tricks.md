...
使用命令 `:%s/foo/bar/g` 全局查找替换后，Vim 会将光标移到最后一个匹配位置。  
如果想回到查找前的位置，最简单的使用 `Ctrl+O` 进行跳转。

以命令行的方式实现，可以再替换命令后追加 <code>``</code> 或者 `''` 命令。  
前者会返回到原来的位置，后者返回到原位置所在行的第一个非空白符位置。
...
_备注：上面的 `''` 可以用 `norm!''` 代替，这是 **Ex** 命令的语法（查看 `:help range`）_
